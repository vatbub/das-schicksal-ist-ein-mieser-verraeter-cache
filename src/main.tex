...
\setlist[optionlist, 2]{
    label=Option \theoptionlisti.\arabic*:,
    ref=\theoptionlisti.\arabic*
}
\setlist[optionlist, 3]{
    label=Option \theoptionlisti.\theoptionlistii.\arabic*:,
...
\newenvironment{option}[2]{
    ...
    \end{adjustwidth}
    \textit{(Ende von Entscheidung~\ref{\csname DecisionLabel\the\value{DecisionEnvironmentDepth} \endcsname},~Option~\ref{\currentOptionLabel})}
...
\newenvironment{conditional}[2]{
    ...
    \textbf{Bedingung \theconditional:} Wenn Entscheidung~\ref{#1}, Option~\ref{#2}:
        }{
    \textit{(Ende von Bedingung \theconditional.)}
}
