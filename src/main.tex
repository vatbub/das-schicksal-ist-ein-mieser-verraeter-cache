...
\newenvironment{decision}[2]{
    ...
    \begin{enumerate}[label={Option \arabic*:}, font=\bfseries]
    }{
    \end{enumerate}
}

\newcommand{\itemNumber}{\number\numexpr\value{enumi}\relax}
\newenvironment{option}[2]{\item #2 \label{#1}}{\textit{(Ende von Entscheidung \thedecision~Option \itemNumber)}}

\newcounter{conditional}
\newenvironment{conditional}[2]{
    \refstepcounter{conditional}
    \textbf{Bedingung \theconditional:} Wenn Entscheidung~\ref{#1},~\ref{#2}:
        }{
